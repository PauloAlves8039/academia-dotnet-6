using System.Globalization;

internal class Program
{
    private static void Main(string[] args)
    {
        /*
         * 15 - Faça um programa para uma loja de tintas. O programa deverá pedir o tamanho em metros quadrados 
         * da área a ser pintada. Considere que a cobertura da tinta é de 1 litro para cada 3 metros quadrados 
         * e que a tinta é vendida em latas de 18 litros, que custam R$ 80,00. 
         * Informe ao usuário a quantidades de latas de tinta a serem compradas e o preço total.
        */

        Console.WriteLine("***** Calcular Total de Latas de Tinta *****");

        Console.Write("Digite o tamanho da área a ser pintada em metros quadrados: ");
        int tamanho = Convert.ToInt32(Console.ReadLine());

        Console.WriteLine("\n----- Resultado -----");

        CalculaTintasETotal(tamanho);

        Console.ReadKey();
    }

    static void CalculaTintasETotal(int tamanho)
    {
        int latas;

        if (tamanho % 54 == 0)
        {
            latas = tamanho / 54;
        }
        else
        {
            latas = (tamanho / 54) + 1;
        }

        double preco = latas * 80.0;

        Console.WriteLine($"{latas} lata(s)");
        Console.WriteLine($"R$ {preco.ToString("F2", CultureInfo.InvariantCulture)}");
    }
}